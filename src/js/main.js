...
/////////// show target section and hide other aside////////
const navItem = document.querySelectorAll(".nav-item");
const section = document.querySelectorAll("section");
navItem.forEach(function (item, id) {
  item.addEventListener("click", function (e) {
  //   let pageName = e.target.getAttribute("data-view");
  //   window.location.hash = pageName
  //  console.log(window.location.hash);

    // let url = new URL(window.location.href).href;
    // let currentUrl = `${url}/${pageName}`;
...
async function LongWeekendDisplay(year) {
  ...
  for (let i = 0; i < LongWeekendData.length; i++) {
    ...
    while (currentDate <= endDate) {
      const isWeekend =
        currentDate.getDay() === 0 || currentDate.getDay() === 6; // saturday or sunday
      days += `
